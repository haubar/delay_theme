...
class Toujours_Theme_Plugin_Enhancements {
	...
	static function init() {
		...
		if ( ! $instance ) {
			$instance = new Toujours_Theme_Plugin_Enhancements;
		}
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				...
				'message' => sprintf(
					esc_html__( 'The %1$s is necessary to use some of this theme&rsquo;s features, including: ', 'toujours' ),
					'<strong>' . esc_html__( 'Jetpack plugin', 'toujours' ) . '</strong>' ),
	...
	function get_theme_dependencies() {
		...
		if ( current_theme_supports( 'site-logo' ) ) :
			$dependencies['logo'] = array(
				'name' => esc_html__( 'Site Logo', 'toujours' ),
				'slug' => 'site-logo',
				'url'  => '',
				'module' => 'none',
			);
		...
		if ( current_theme_supports( 'featured-content' ) ) :
			$dependencies['featured-content'] = array(
				'name' => esc_html__( 'Featured Content', 'toujours' ),
		...
		if ( current_theme_supports( 'jetpack-social-menu' ) ) :
			$dependencies['social-menu'] = array(
				'name' => esc_html__( 'Social Menu', 'toujours' ),
		...
		if ( current_theme_supports( 'nova_menu_item' ) ) :
			$dependencies['menus'] = array(
				'name' => esc_html__( 'Menus', 'toujours' ),
				'slug' => 'nova_menu_item',
				'url'  => '',
		...
		if ( current_theme_supports( 'jetpack-comic' ) ) :
			$dependencies['comics'] = array(
				'name' => esc_html__( 'Comics', 'toujours' ),
				'slug' => 'jetpack-comic',
				'url'  => '',
				'module' => 'custom-content-types',
			);
		...
		if ( current_theme_supports( 'jetpack-testimonial' ) ) :
			$dependencies['testimonials'] = array(
				'name' => esc_html__( 'Testimonials', 'toujours' ),
		...
		if ( current_theme_supports( 'jetpack-portfolio' ) ) :
			$dependencies['portfolios'] = array(
				'name' => esc_html__( 'Portfolios', 'toujours' ),
		...
		if ( current_theme_supports( 'jetpack-content-options' ) ) :
			$dependencies['content-options'] = array(
				'name' => esc_html__( 'Content Options', 'toujours' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			'none'                 => esc_html__( 'no specific module needed', 'toujours' ),
			'custom-content-types' => esc_html__( 'Custom Content Types module', 'toujours' ),
		);
	...
	function admin_notices() {
		...
		if ( get_user_meta( get_current_user_id(), 'toujours_jetpack_admin_notice', true ) === 'dismissed' ) {
			return;
		}
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please activate %1$s. %2$s', 'toujours' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate', 'toujours' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' Please install %1$s. %2$s', 'toujours' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install', 'toujours' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				esc_html__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'toujours' ),
...
add_action( 'admin_head', array( 'Toujours_Theme_Plugin_Enhancements', 'init' ) );

function enqueue_scripts() {
	// Add the admin JS if the notice has not been dismissed
	if ( is_admin() && get_user_meta( get_current_user_id(), 'toujours_jetpack_admin_notice', true ) !== 'dismissed' ) {

		// Adds our JS file to the queue that WordPress will load
		wp_enqueue_script( 'toujours_jetpack_admin_script', get_template_directory_uri() . '/inc/plugin-enhancements.js', array( 'jquery' ), '20160624', true );

		// Make some data available to our JS file
		wp_localize_script( 'toujours_jetpack_admin_script', 'toujours_jetpack_admin', array(
			'toujours_jetpack_admin_nonce' => wp_create_nonce( 'toujours_jetpack_admin_nonce' ),
		));
	}
}
...
function dismiss_admin_notice() {
	...
	if ( ! isset( $_POST['toujours_jetpack_admin_nonce'] ) || ! wp_verify_nonce( $_POST['toujours_jetpack_admin_nonce'], 'toujours_jetpack_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'toujours' ) );
	}
	// Store the user's dimissal so that the notice doesn't show again
	update_user_meta( get_current_user_id(), 'toujours_jetpack_admin_notice', 'dismissed' );
	// Send success message
	wp_send_json( array(
		'status' => 'success',
		'message' => __( 'Your request was processed. See ya!', 'toujours' )
	) );
}
add_action( 'wp_ajax_toujours_jetpack_admin_notice', 'dismiss_admin_notice' );
